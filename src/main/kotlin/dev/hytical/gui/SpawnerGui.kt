...
class SpawnerGui(private val ctx: ServiceContext) {
	...
	fun open(viewer: Player, target: Player) {
		...
		val backItem = createClickableItem(XMaterial.REDSTONE_BLOCK, messageService.getRaw("spawner_back"), messageService) {
			if (viewer == target) {
